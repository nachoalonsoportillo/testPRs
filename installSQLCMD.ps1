...
Start-Process -FilePath "C:\Program Files\Microsoft SQL Server\Client SDK\ODBC\170\Tools\Binn\sqlcmd.exe" -ArgumentList @("-S sql-adl-nonsecured.database.windows.net", "-U sqladminuser", "-P ThisIsNotVerySecure!", "-d sqldbadl-nonsecured", "-Q ""declare @i int = 1; declare @stmt varchar(200); while (@i <= 150000) begin set @i += 1; set @stmt = 'drop table if exists t' + cast(@i as varchar(10)) + '; create table t' + cast(@i as varchar(10)) + '(c1 int IDENTITY(1,2), c2 varchar(10), c3 bigint not null, c4 bit, c5 datetime2, c6 time, c7 nchar(10), c8 tinyint, c9 decimal, c10 char(25), c11 datetimeoffset, c12 varbinary(100), c13 smalldatetime, c14 date, c15 float, c16 money)'; exec (@stmt); end""", "-o $logLoc\SQLCMD.log") -RedirectStandardOutput $logLoc\stdout.log -RedirectStandardError $logLoc\stderr.log -Wait
Trace-Log "Finished."
